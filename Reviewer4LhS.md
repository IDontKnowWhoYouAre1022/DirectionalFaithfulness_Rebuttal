...
**Interpretation:** This is an example of grid-induced local collapse. Both trajectories use only two tokens and are almost entirely assigned to token $A$. The $0^\circ$ trajectory changes token only at its final three samples, while the $90^\circ$ trajectory differs only at its first sample. Consequently, $H=0.16$ and $C=0.92$. More importantly, the two representative vectors differ primarily in u1; the dashed paths remain constant in u2,...,u5. Thus, the scalar grid registers a large one-coordinate transition while discarding most of the distinct multi-coordinate slopes present in the continuous paths. This produces the large $59.5^\circ$ angular error. The axis-aligned scalar grid is poorly matched to this curved, correlated learned latent geometry. This is consistent with the paper’s argument that structured discretization or nominal utilization does not ensure directional preservation. 
